...
def init_db():
    with sqlite3.connect('parking.db') as conn:
        ...
        conn.execute('CREATE TABLE IF NOT EXISTS parking_centers (id INTEGER PRIMARY KEY, name TEXT, user_id INTEGER, FOREIGN KEY(user_id) REFERENCES users(id))')
        conn.execute('CREATE TABLE IF NOT EXISTS cars (id INTEGER PRIMARY KEY, car_number TEXT UNIQUE, parking_center_id INTEGER, FOREIGN KEY(parking_center_id) REFERENCES parking_centers(id))')
        conn.commit()
...
def manage_car_in_center(center_id):
    ...
    with sqlite3.connect('parking.db') as conn:
        ...
        if action == 'add':
            try:
                cursor.execute('INSERT INTO cars (car_number, parking_center_id) VALUES (?, ?)', (car_number, center_id))
                conn.commit()
